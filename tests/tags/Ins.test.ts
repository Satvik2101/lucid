...
describe("Ins", () => {
    ...
    it("should set tag name as ins", () => {
        expect(tag.tagName).toBe("ins");
    });

    it("should set all attributes in factory constructor", () => {
        const attr = {
			"cite": faker.lorem.word(),
        ...
        tag = Ins.withAttributes(attr);
        
        expect(tag.getAttr("cite")).toBe(attr["cite"]);
		expect(tag.getAttr("datetime")).toBe(attr["datetime"]);
